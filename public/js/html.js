    ...
    if (state.todos.length === 0) {
        ...
        state.todos.forEach(todo => {
            if (!todo.completed) html += `<li class="list-group-item" id="${todo.id}"><span class="text">${todo.text}</span> <span class="delete">&#9985;</span></li>`
                ...
                html += `<li class="list-group-item text completed" id="${todo.id}"><span class="text">${todo.text}</span> <span class="delete">&#9985;</span></li>`
            }
        });
        return html;
        ...
        state.todos.filter(todo => !todo.completed).forEach(todo => {
            html += `<li class="list-group-item text" id="${todo.id}"><span class="text">${todo.text}</span> <span class="delete">&#9985;</span></li>`
        });
        return html;
    }
}